...
#!/usr/bin/python
"""
Cross validation Partition 10 fold
@Author Yi Zhu
Upated 03/31/2017
"""
...
def testSubTokenAutoPos(corpus, sub_corpus, seq_out, conll_out, use_pos, use_brown):
    fout_seq = codecs.open(seq_out, "w", "utf-8")
    fout_conll = codecs.open(conll_out, "w", "utf-8")
    ...
    for i, sent in enumerate(corpus):
        ...
        for j, word in enumerate(sent):
            ...
            assert sent[j][0] == sub_corpus[i][j][0]
            sent[j][1] = sub_corpus[i][j][1]
        res = convert_sentence(sent, use_pos, use_brown)
        fout_seq.write(res + u"\n")
        sent_conll = u"\n".join([u"\t".join(line) for line in sent]) + u"\n\n"
        fout_conll.write(sent_conll)
...
def crossVal(inputf):
    ...
    len_sent = len(corpus)
    part_lim = len_sent / 10 + 1
    train_out = 'cross_train_allproj_'
    dev_out = 'cross_dev_autopos_'
    for i in xrange(10):
        part_dev = corpus[i * part_lim: (i + 1) * part_lim]
        part_train = corpus[: i * part_lim] + corpus[(i + 1) * part_lim:]
        trainNp2p(part_train, train_out + str(i), train_out + str(i) + '_conv', True, False)
        testSubTokenAutoPos(part_dev, part_dev, dev_out + str(i), dev_out + str(i) + '_conv', True, False) 
