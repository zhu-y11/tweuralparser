...
#!/usr/bin/python
"""
Convert parsing result to conll format
@Author Yi Zhu
Upated 02/20/2016
"""
...
def convet2CONLL(input_file, output_file):
    ...
    with open(input_file, 'r') as f:
        for line in f:
            ...
            dp_buffer = [tuple([i] + b.split('/')) for i, b in enumerate(dp_buffer)]
            for op_pair in ops:
                if op_pair == 'SHIFT':
		    ...
		    rel = op_pair[s_idx + 1:]
                    if op == 'LEFT_ARC':
                        mod = dp_stack[-2]
                        head = dp_stack[-1]
                        dp_map[mod[0]] = '%d\t%s\t_\t_\t%s\t_\t%d\t%s\t_\t_'%(mod[0], mod[1], mod[-1], head[0], rel)
                        dp_stack.pop(-2)
                    elif op == 'RIGHT_ARC':
                        mod = dp_stack[-1]
                        head = dp_stack[-2]
